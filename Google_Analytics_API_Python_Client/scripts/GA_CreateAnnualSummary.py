...
def create_report(startdate):
    ...
    top_nodetails = [] #Creating list of parent directories with missing information from GA.
    ...
    # WRITING GOOGLE ANALYTICS DATA TO GA_TopLevelCollectionsReportYYYYMMDD.csv.
    
    #----------------------------------------------------------------------------- USER EDIT ---------
    # Open the csv file to which the collections report will be written to.
    # DEAR USER: Enter the filepath and the corresponding filename for a csv file to which the collection
    #            report data will be written to.
    GARPathTop = 'C:\Home\\Digital-Archive-Tools\\Google_Analytics_API_Python_Client\\scripts\\' + startdate
    GARFileTop = 'GA_TopLevelCollectionsReport' + datetime.datetime.today().strftime('%Y%m%d') + '.csv'
    f = open(GARPathTop.strip('\\') + '\\' + GARFileTop, 'wt')

    # Open GA_CollectionsReport.csv file for appending.
    g = open(GARPathTop.strip('\\') + '\\' + GARFileTop, 'a')
    writer = csv.writer(g, dialect='excel', lineterminator='\n')

    # Write headers for GA_CollectionsReport.csv.
    headerwriter = csv.DictWriter(g, fieldnames = ["COLLECTION MACREPO ID", "WEBSITE URL",DIMENSION, METRIC1 + " (TOTAL)",METRIC2 + " (TOTAL)", TIMERANGE])
    headerwriter.writeheader()

    # Creating list of unique top-level collections.
    unique_top = []
    [unique_top.append(top) for top in topleveldir if top not in unique_top]

    for top in unique_top:

        totalusers = 0
        totalpageviews = 0
        line = []
        line.append(top) #Appending collection MacRepo ID.

        if top not in IDList or top == 0:
            top_nodetails.append(top) #Creating list of top-level directories that are not listed as a MacRepo ID.
            
        else:
            pass

        for topnoid in top_nodetails: #Appending "N/A" for collection's URL and pagetitles if it is not listed as a MacRepo ID.
        ...
        with open(GARPath.strip('\\') + '\\' + GARFile, 'r') as lookupfile2:
            ...
            reader5 = csv.reader(lookupfile2, delimiter=",")

            for row in reader5:

                if row[0] == top: #Appending MacRepo ID details for the collection.
                    line.append(row[1]) #Appending corresponding URL.
                    line.append(row[2]) #Appending corresponding dimension (page title).
                ...
                if row[8] == top and row[5] == "1": #Tabulating the total numbers of users and pageviews for each top-level directory.
                    usersnum = int(row[3])
                    pageviewsnum = int(row[4])
                    totalusers = totalusers + usersnum 
                    totalpageviews = totalpageviews + pageviewsnum
    ...
    print ("Success. Your Google Analytics TopLevelCollections report has been written to " + GARFileTop + " in " + GARPathTop)
